...
onMounted(() => {
  console.log('HelloWorld component mounted, setting up event listener for newVideoDetected')
  EventsOn('newVideoDetected', handleNewVideo)
  
  // Also listen for app restore event
  EventsOn('app-restored-from-tray', () => {
    console.log('HelloWorld: App restored from tray')
  })
})

onUnmounted(() => {
  console.log('HelloWorld component unmounting, removing event listeners')
  EventsOff('newVideoDetected')
  EventsOff('app-restored-from-tray')
})

function handleNewVideo(data) {
  console.log('HelloWorld: New video detected event received:', data)
